<footer>
  <div class="container-xxl h-100 pb-2">
    <div class="row align-items-center h-100 text-md-start text-center">
      <div class="col-md-7 col-12 pt-md-0 pt-2">
        <img class="modern-house-logo" src="{{ asset('storage/logo/logo-white.png') }} " alt="Modern House Logo">
      </div>
      <div class="col-md-2 col-6">
        <p class="small">601 1st Floor Office, Edenderry Business Campus<br/>Edenderry, Co.Offaly, Ireland</p>
        <p class="small">@lang('modern house registration number'): 776549</p>
        <p class="small">@lang('VAT'): IE4357778BH</p>
      </div>
      <div class="col-md-2 col-6">
        <p class="small">
          <i class="bi bi-envelope-fill text-white"></i> <a href="mailto:info@modern-house.ie"
                                                            class="text-white text-decoration-none">info@modern-house.ie</a>
        </p>
        <p class="small">
          <i class="bi bi-telephone text-white"></i> <a href="https://wa.link/by73e7" target="_blank"
                                                        class="text-white text-decoration-none">+353
            83 854 9155</a>
        </p>
        <p class="small">
          <i class="bi bi-telephone text-white"></i> <a href="https://wa.link/2ojact" target="_blank"
                                                        class="text-white text-decoration-none">+353 83 480 1610</a>
        </p>
        <p class="small">
          <i class="bi bi-building text-white"></i> <a href="tel:0469755198" target="_blank"
                                                       class="text-white text-decoration-none">04 697 55198</a>
        </p>
      </div>
      <div class="col-md-1 col-12 d-flex flex-md-column justify-content-center align-items-md-end">
        <div class="p-2">
          <a href="https://www.tiktok.com/@modern_house_ire" class="nav-link" target="_blank"><i
              class="bi bi-tiktok text-white"></i></a>
        </div>
        <div class="p-2">
          <a href="https://www.instagram.com/modernhouse_ireland/" class="nav-link" target="_blank"><i
              class="bi bi-instagram text-white"></i></a>
        </div>
        <div class="p-2">
          <a href="https://www.facebook.com/profile.php?id=61571019609255" class="nav-link" target="_blank"><i
              class="bi bi-facebook text-white"></i></a>
        </div>
        <div class="p-2">
          <a href="mailto:info@modern-house.ie" class="nav-link" target="_blank"><i
              class="bi bi-envelope-fill text-white"></i></a>
        </div>
      </div>
      <hr class="m-0">
      <div class="d-flex justify-content-center align-content-center flex-column text-center">
        <div>
          <img class="img-fluid" width="30" src="{{ asset('storage/ireland.png') }}" alt="">
        </div>
        <div>
          <p class="small">&copy; {{ config('app.name') }} {{  date ('Y') }}
            | @lang('all rights reserved')</p>
        </div>
        <div>
          <p class="small">@lang('Developed by') <a href="https://slmedia.lv" class="text-white" target="_blank">S&L
              Media</a></p>
        </div>
      </div>
    </div>
  </div>
</footer>
