      ...
      <div class="col-lg-7 col-12">
        <form action="{{ route('admin.promo-modal.update', ['locale' => app()->getLocale()]) }}" method="POST"
          ...
          <div class="form-check mb-3">
            <input type="checkbox" class="form-check-input" id="promo-modal-is-enabled" name="is_enabled" value="1"
                   @checked(old('is_enabled', $promoModal->is_enabled))>
            <label class="form-check-label" for="promo-modal-is-enabled">Rādīt reklāmas logu sākumlapā</label>
          </div>
          <div class="row">
            ...
            <div class="col-6 mb-3">
              ...
              <input type="date" class="form-control" id="promo-modal-ends-at"
                     name="ends_at" value="{{ old('ends_at', $promoModal->ends_at?->format('Y-m-d')) }}">
            </div>
          </div>
          <div class="mb-3">
            <label for="promo-modal-title" class="form-label">Virsraksts</label>
            <input type="text" class="form-control" id="promo-modal-title"
                   name="title" value="{{ old('title', $promoModal->title) }}">
          </div>
          <div class="mb-3">
          ...
          <div class="row">
            <div class="col-6 mb-3">
              ...
              <input type="text" class="form-control" id="promo-modal-cta-label"
                     name="cta_label" value="{{ old('cta_label', $promoModal->cta_label) }}">
            </div>
            <div class="col-6 mb-3">
              <label for="promo-modal-cta-url" class="form-label">Pogas saite</label>
              <input type="text" class="form-control" id="promo-modal-cta-url"
                     name="cta_url" value="{{ old('cta_url', $promoModal->cta_url) }}">
            </div>
          </div>
          <div class="mb-3">
            <label class="form-label">Bilde</label>
            <x-file-upload :name="'promo-modal-image'" :required="$promoModal->image_filename ? 'false' : 'true'"
                           :files="$promoModal->image_filename ? json_encode([App\Models\PromoModal::IMAGE_DIRECTORY.'/'.$promoModal->image_filename]) : null"/>
          </div>
          <button type="submit" class="btn btn-success">Saglabāt</button>
        </form>
      </div>
